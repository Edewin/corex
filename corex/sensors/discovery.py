...
class SensorDiscovery:
    def run_discovery(
        ...
        baselines = {(c.id, s.sensor_id): s.value for c, s in candidates}
        ...
        time.sleep(duration_secs + 0.5)  # Allow extra time for measurement
        updated = get_all_lm_components(cpu_component=lm_components[0])
